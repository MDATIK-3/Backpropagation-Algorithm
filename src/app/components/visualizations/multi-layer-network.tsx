...
export default function MultiLayerNetwork({
  ...
  useEffect(() => {
    ...
    const layers = [
      ...
      [
        ...
        { x: 80, y: (2 * height) / 3, label: "x₂", value: inputs[1], type: "input" },
      ],
      // Hidden layer
      [
        { x: 280, y: height / 3, label: "h₁", value: a2[0], type: "hidden" },
        { x: 280, y: (2 * height) / 3, label: "h₂", value: a2[1], type: "hidden" },
      ],
      // Output layer
      [
        { x: 480, y: height / 3, label: "y₁", value: a3[0], type: "output" },
        { x: 480, y: (2 * height) / 3, label: "y₂", value: a3[1], type: "output" },
      ],
    ]
    ...
    for (let i = 0; i < layers[0].length; i++) {
      for (let j = 0; j < layers[1].length; j++) {
        ...
        ctx.strokeStyle = isActive ? "#fbbf24" : connectionColors[connectionIndex % connectionColors.length]
        ...
        ctx.fillStyle = connectionColors[connectionIndex % connectionColors.length]
    ...
    layers.forEach((layer, layerIndex) => {
      layer.forEach((node, nodeIndex) => {
        const isActive = isAnimating && animationStep >= layerIndex * 4 + nodeIndex
  ...
  }, [inputs, weightsLayer1, weightsLayer2, targets, animationStep, isAnimating, width, height])
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <div className="mt-4 text-sm text-gray-600 text-center max-w-2xl">
        ...
        <p className="text-xs">
          2 inputs → 2 hidden neurons → 2 outputs. Weight notation: θ⁽ˡ⁾ᵢⱼ where l=layer, i=to neuron, j=from neuron.
        </p>
      </div>
    </div>
  )
