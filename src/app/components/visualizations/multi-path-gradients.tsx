...
export default function MultiPathGradients({ inputs, weightsLayer1, weightsLayer2, targets }: MultiPathGradientsProps) {
  ...
  // Two paths for θ⁽¹⁾₁₁
  const path1 = dJ_da3_1 * da3_dz3_1 * dz3_da2_1_from_output1 * da2_dz2_1 * dz2_dtheta11
  const path2 = dJ_da3_2 * da3_dz3_2 * dz3_da2_1_from_output2 * da2_dz2_1 * dz2_dtheta11
  ...
  return (
    <div className="space-y-6">
      <Card>
        ...
        <CardContent>
          <p className="text-gray-700 mb-4">
            In multi-layer networks, a single weight can affect multiple outputs through different paths. We must sum
            the gradients from all paths to get the total gradient.
          </p>

          <div className="bg-blue-50 p-4 rounded-lg mb-4">
            <p className="font-semibold text-blue-800 mb-2">Example: Computing ∂J/∂θ⁽¹⁾₁₁</p>
            <p className="text-blue-700 text-sm">
              θ⁽¹⁾₁₁ affects both output neurons through hidden neuron h₁, creating two gradient paths that must be
              summed.
            </p>
          </div>
        </CardContent>
      ...
      <Card className="border-green-500">
        <CardHeader>
          <CardTitle className="text-lg text-green-700">Path 1: θ⁽¹⁾₁₁ → h₁ → y₁</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            ...
            <div className="bg-green-100 p-3 rounded">
              <p className="font-semibold text-center">Path 1 Result: {path1.toFixed(6)}</p>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="border-orange-500">
        <CardHeader>
          <CardTitle className="text-lg text-orange-700">Path 2: θ⁽¹⁾₁₁ → h₁ → y₂</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            ...
            <div className="bg-orange-100 p-3 rounded">
              <p className="font-semibold text-center">Path 2 Result: {path2.toFixed(6)}</p>
            </div>
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent>
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-center space-x-4">
              <Badge className="bg-green-100 text-green-800 px-4 py-2">{path1.toFixed(6)}</Badge>
              <Plus className="w-4 h-4 text-gray-500" />
              <Badge className="bg-orange-100 text-orange-800 px-4 py-2">{path2.toFixed(6)}</Badge>
              <span className="text-gray-500">=</span>
              <Badge className="bg-purple-100 text-purple-800 px-4 py-2 text-lg">{totalGradient.toFixed(6)}</Badge>
            </div>
