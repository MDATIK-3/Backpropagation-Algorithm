...
export default function GeneralizingPage() {
  ...
  const [currentView, setCurrentView] = useState<"network" | "gradients">("network")
  ...
  const z3 = [
    networkParams.weightsLayer2[0][0] * a2[0] + networkParams.weightsLayer2[0][1] * a2[1],
    networkParams.weightsLayer2[1][0] * a2[0] + networkParams.weightsLayer2[1][1] * a2[1],
  ]
  const a3 = [1 / (1 + Math.exp(-z3[0])), 1 / (1 + Math.exp(-z3[1]))]

  const errors = [a3[0] - networkParams.targets[0], a3[1] - networkParams.targets[1]]
  ...
  return (
    ...
    >
      <div className="space-y-12">
              ...
              <div className="grid lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2">
                  <p className="text-gray-700 leading-relaxed mb-4">
                    Moving beyond the simple toy example, we now explore networks with multiple neurons in each layer.
                    This introduces weight matrices, multiple gradient paths, and the need to sum gradients from
                    different routes through the network.
                  </p>
                  <div className="bg-purple-50 p-4 rounded-lg border-l-4 border-purple-500">
                    <p className="font-semibold text-purple-800 mb-2">New Challenges:</p>
                    <ul className="text-purple-700 text-sm space-y-1">
                      <li>• Weight matrices instead of individual weights</li>
                      <li>• Multiple paths from each weight to the cost function</li>
                ...
                <div>
                  <div className="space-y-3">
                    <Badge className="bg-green-100 text-green-800 w-full justify-center py-2">2 Input Neurons</Badge>
                    <Badge className="bg-yellow-100 text-yellow-800 w-full justify-center py-2">2 Hidden Neurons</Badge>
                    <Badge className="bg-red-100 text-red-800 w-full justify-center py-2">2 Output Neurons</Badge>
                    <Badge className="bg-purple-100 text-purple-800 w-full justify-center py-2">8 Parameters</Badge>
                  </div>
                </div>
              </div>
            </CardContent>
            ...
            <CardContent>
              <div className="space-y-4">
                <p className="text-gray-700">
                  With multiple outputs, we use the mean squared error across all output neurons:
                </p>
                ...
                <div className="grid md:grid-cols-3 gap-4 text-center">
                  <div>
                    <Badge className="bg-blue-100 text-blue-800 mb-2">Current Cost</Badge>
                    <p className="font-mono text-lg">{cost.toFixed(6)}</p>
                  </div>
                  <div>
                    <Badge className="bg-green-100 text-green-800 mb-2">Output 1 Error</Badge>
                    <p className="font-mono text-lg">{errors[0].toFixed(4)}</p>
                  </div>
                  <div>
                    <Badge className="bg-red-100 text-red-800 mb-2">Output 2 Error</Badge>
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <Slider
                        ...
                        onValueChange={(value) =>
                          setNetworkParams((prev) => ({ ...prev, inputs: [value[0], prev.inputs[1]] }))
                        }
                        max={2}
                        min={-2}
                        step={0.1}
                    ...
                    <div>
                      ...
                      <Slider
                        ...
                        onValueChange={(value) =>
                          setNetworkParams((prev) => ({ ...prev, inputs: [prev.inputs[0], value[0]] }))
                        }
                        max={2}
                        min={-2}
                        step={0.1}
                    ...
                    <div>
                      ...
                      <Slider
                        ...
                        onValueChange={(value) =>
                          setNetworkParams((prev) => ({ ...prev, targets: [value[0], prev.targets[1]] }))
                        }
                        max={1}
                        min={0}
                        step={0.1}
                    ...
                    <div>
                      ...
                      <Slider
                        ...
                        onValueChange={(value) =>
                          setNetworkParams((prev) => ({ ...prev, targets: [prev.targets[0], value[0]] }))
                        }
                        max={1}
                        min={0}
                        step={0.1}
        ...
        {currentView === "network" ? (
          <section>
            <h2 className="text-3xl font-bold text-center mb-8">Interactive Weight Matrix Controls</h2>
            <WeightMatrixControls
              weightsLayer1={networkParams.weightsLayer1}
              weightsLayer2={networkParams.weightsLayer2}
              onWeightsLayer1Change={(weights) => setNetworkParams((prev) => ({ ...prev, weightsLayer1: weights }))}
              onWeightsLayer2Change={(weights) => setNetworkParams((prev) => ({ ...prev, weightsLayer2: weights }))}
            />
          </section>
        ) : (
          <section>
            <h2 className="text-3xl font-bold text-center mb-8">Multi-Path Gradient Calculations</h2>
            <MultiPathGradients
              inputs={networkParams.inputs}
              weightsLayer1={networkParams.weightsLayer1}
              weightsLayer2={networkParams.weightsLayer2}
        ...
        <section>
          <Card>
            <CardHeader>
              <CardTitle className="text-center">Key Insights from Multi-Layer Networks</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="grid md:grid-cols-3 gap-6">
                <div className="text-center">
                  <Badge className="bg-blue-100 text-blue-800 mb-3 px-4 py-2">Weight Matrices</Badge>
                  <p className="text-sm text-gray-700">
                    Weights are organized in matrices where θ⁽ˡ⁾ᵢⱼ represents the weight from neuron j in layer l-1 to
                    neuron i in layer l.
                  </p>
                </div>
                <div className="text-center">
                  <Badge className="bg-green-100 text-green-800 mb-3 px-4 py-2">Multiple Paths</Badge>
                  <p className="text-sm text-gray-700">
                    Each weight can affect multiple outputs through different paths, requiring us to sum gradients from
                    all possible routes.
                  </p>
                </div>
                <div className="text-center">
                  <Badge className="bg-purple-100 text-purple-800 mb-3 px-4 py-2">Scalable Algorithm</Badge>
                  <p className="text-sm text-gray-700">
                    The backpropagation algorithm scales efficiently to networks of any size using matrix operations and
